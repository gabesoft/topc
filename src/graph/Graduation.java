...
public class Graduation {
  ...
  public String moreClasses(String classesTaken, String[] requirements) {
    ...
    for (int i = 0; i < requirements.length; i++) {
      ...
      assert classesPerReq[i] <= maxc : "invalid class count";
      for (; j < req.length(); j++) {
    ...
    int c = 0;
    classes = new char[maxc];
    reqGraph = new boolean[n][maxc];
  ...
  boolean matchReq(int cls, int[] clsMatch, int[] reqCount) {
    ...
    while (from[curr] != curr) {
      assert reqGraph[match][curr] : "invalid match: " + match + " -> " + curr;
      clsMatch[curr] = match;
      curr = from[curr];
      match = newMatch[curr];
    }
    ...
    assert reqCount[match] <= classesPerReq[match] : "invalid req count";
    return true;
  }
