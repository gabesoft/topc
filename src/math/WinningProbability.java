...
public class WinningProbability {
  ...
  public double probability(int prevWins, int prevLosses, int gamesLeft, int mustWin) {
    ...
    for (int i = 0; i < ACC + 1; i++) {
      ...
      double pprev = calcProb(p, prevWins, prevWins + prevLosses, false);
      double pnext = calcProb(p, mustWin, gamesLeft, true);
      den += pprev;
      num += pprev * pnext;
    }
  ...
  double calcProb(double pwin, int wins, int total, boolean atLeast) {
    ...
    for (int j = 1; j < m; j++) {
      probs[0][j] = probs[0][j - 1] * (1.0 - pwin);
      for (int i = 1; i < n; i++) {
        probs[i][j] += probs[i - 1][j - 1] * pwin;
        probs[i][j] += probs[i][j - 1] * (1.0 - pwin);
      }
    }
